...
class PatientIdentity(BaseModel):
    """Optional patient demographics for documents where identity is
    discoverable but not always present.

    Distinct from `Demographics` (which is required on `IntakeForm` and
    enforces a non-empty given/family name). Every field here is
    optional — a fax cover with no patient header, an HL7 ADT-A08 with
    the PID-5 component left blank, or a referral letter that reads
    "the patient" throughout will all extract to `None` fields rather
    than failing the schema. The new-patient flow consumes whatever
    fields ARE populated and asks the user to fill in the rest.

    `source_citation` is also optional — the parser path (HL7) doesn't
    have a meaningful page-or-section locator for an MRN that came off
    a single PID-3 component.
    """
...
class ReferralLetter(BaseModel):
    ...
    patient_identity: PatientIdentity | None = Field(
        default=None,
        description=(
            "Patient name / DOB / sex / address / phone as printed in the "
            "letter header or salutation. Optional — referrals that read "
            "'the patient' throughout extract this field as null. Used by "
            "the new-patient-from-document flow; never written back to "
            "OpenEMR (the patient already exists if the letter resolves)."
        ),
    )
...
class Hl7Message(BaseModel):
    ...
    patient_identity: PatientIdentity | None = Field(
        default=None,
        description=(
            "Patient name (PID-5) / DOB (PID-7) / sex (PID-8) / address "
            "(PID-11) / phone (PID-13) parsed off the PID segment. "
            "Used by the new-patient-from-document flow only — Phase-3 "
            "persistence intentionally does NOT write these back to "
            "OpenEMR for ADT-A08, since clobbering an existing chart's "
            "demographics from a registration update is exactly the "
            "failure mode the schema design was guarding against."
        ),
    )
...
class FaxPacket(BaseModel):
    ...
    patient_identity: PatientIdentity | None = Field(
        default=None,
        description=(
            "Patient name / DOB / sex / address / phone as printed on "
            "the patient face sheet. Optional — covers without a face "
            "sheet (cover-only transmittals) extract this as null. Used "
            "by the new-patient-from-document flow."
        ),
    )
