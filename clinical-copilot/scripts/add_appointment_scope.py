"""Add the calendar/scheduling scopes to an EXISTING agent_forge_seed
client without re-registering (which would mint a new client_id/secret
pair and force an .env edit).

Use this when you have a Week 1/2 .env wired up and don't want to rotate
the seed client credentials. Otherwise just re-run `register_seed_client.py`
— the SCOPES list there now includes the new appointment scopes.

Run: PYTHONPATH=. uv run python scripts/add_appointment_scope.py
"""
...
NEW_SCOPES = [
    # Standard non-FHIR API (POST /api/patient/{pid}/appointment) — the
    # write path used by app/fhir/writer.py:write_appointment.
    "user/appointment.cruds",
    # FHIR Appointment read — used by app/fhir/extras.py:get_calendar_today
    # to overlay scheduled appointments on the Today's Calendar tab.
...
def main() -> None:
    ...
    if not missing:
        print("  ✓ Appointment scopes already present — nothing to do.")
        return
