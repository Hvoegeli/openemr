...
- **Audit log (in-memory ring buffer post-MVP, Postgres deferred to Sunday).** A 200-deep `RequestTrace` ring buffer ([clinical-copilot/app/observability.py](clinical-copilot/app/observability.py)) captures who/what/when per request: session ID, tool name, args, sources returned, validator outcome, latency, token + $ cost. Surfaced through `/observability` + `/api/traces` for the in-app dashboard. The persistent Postgres `audit_events` table is still Sunday work; HIPAA compliance posture in production needs the persistent store + retention policy. OpenEMR's FHIR layer doesn't audit-log API calls consistently ([AUDIT.md §5.1](AUDIT.md#51-audit-logging--partial)) so we own this entirely.
